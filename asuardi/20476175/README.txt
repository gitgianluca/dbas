...
Oracle Database 11g Release 11.2.0.4.160119

PARALLEL QUERY Patch for Bug# 20476175 for Solaris-64 Platforms
...
Released: Thu Mar 10 12:04:09 2016
  
This document describes how you can install the PARALLEL QUERY overlay patch for bug#  20476175 on your Oracle Database 11g Release 11.2.0.4.160119
...
1.	Ensure that the Oracle home on which you are installing the patch or from which you are rolling back the patch is Oracle Database 11g Release 11.2.0.4.160119.
 
2.	Ensure that 11g Release 11.2.0.4.160119 Patch Set Update (PSU) 21948347 is already applied on the Oracle Database.
...
7.	(Only for Installation) Maintain a location for storing the contents of the patch ZIP file. In the rest of the document, this location (absolute path) is referred to as <PATCH_TOP_DIR>. Extract the contents of the patch ZIP file to the location (PATCH_TOP_DIR) you have created above. To do so, run the following command:
	$ unzip -d <PATCH_TOP_DIR>  p20476175_11204160119_SOLARIS64.zip 


8.	(Only for Installation) Determine whether any currently installed interim patches conflict with this patch 20476175 as shown below:
	$ cd <PATCH_TOP_DIR>/20476175
	$ opatch prereq CheckConflictAgainstOHWithDetail -ph ./
	
	The report will indicate the patches that conflict with this patch and the patches for which the current 20476175 is a superset.
...
The following are the bugs fixed by this patch:
  20476175: CURSOR IS NOT SHARED BY A DIFFERENT SESSION IF _FIX_CONTROL IS USED IN OPT_PARAM
